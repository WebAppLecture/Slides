			...
			if(node == "pre"){
				...
				if(!o.parent().hasClass("snippet-wrap")){
					...
					var controls = "<div class='snippet-menu sh_sourceCode' style='display:none;'><pre>"
								  ...
								  +"<a class='snippet-text' href='#'><img src='icons/text.png' alt='text'/></a>"
								  +"<a class='snippet-window' href='#'><img src='icons/popup.png' alt='pop-up'/></a>"
								 // +"<a class='snippet-zoom' href='#'><img src='icons/zoom.png' alt='zoom'/></a>"
								  +"<a class='snippet-window-zoom' href='#'><img src='icons/popzoom.png' alt='pop-up-zoom'/></a>"
								  +"</pre></div>";
					...
					o.parent().find("a.snippet-text").click(function(){
						...
						if(txt.is(':visible')){
							$(this).html("<img src='icons/html.png' alt='html'/>");
						} else {
							$(this).html("<img src='icons/text.png' alt='text'/>");
						}
						$(this).blur();
						return false;
					});
					...
					// click event for popup-zoom view
					o.parent().find("a.snippet-window-zoom").click(function(){
						var sfh = $(this).parents('.snippet-wrap').find('.snippet-formatted');
						var txt = sfh.html();
						snippetZoomPopup(txt, sfh.attr('class'), sfh.attr('lang'));
						$(this).blur();
						return false;
					});	

					// click event for zoom view
					o.parent().find("a.snippet-zoom").click(function() {
						var container = o.parents('.snippet-container');
						var height = container.height();
						container.toggleClass('zoom').css({height: height});;
						return false;
					});						
...
function snippetPopup(content) {
	 ...
	 top.consoleRef.document.writeln(
	  '<html><head><title>Codeansicht - ' + location.hash + '</title></head>'
...
// snippet new zoom-window popup function
function snippetZoomPopup(content, classes, lang) {
	 top.consoleRef=window.open('','myconsole','width=900,height=650,left=60,top=70,menubar=0,toolbar=0,location=0,status=0,scrollbars=1,resizable=0');
	 top.consoleRef.document.write(
	  '<!doctype html><html><head><title>(++)Codeansicht - ' + location.hash + '</title>'
	   + '<link rel="stylesheet" href="styles/jquery.snippet.css" /></head>'
	   + '<body bgcolor="#FFF" onLoad="self.focus()" class="snippet-container sh_rand01 zoomwindow">'
	   + '<pre class="' + classes + '" lang="' + lang +'">'
